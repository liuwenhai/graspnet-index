...
from models.graspnet import GraspNet, pred_decode, GraspNetIndex, pred_decodeIndex
from utils.collision_detector import ModelFreeCollisionDetector

import rospy
from geometry_msgs.msg import Point, Quaternion
from gamma_msg_srv.srv import GraspNetIndexList,GraspNetIndexListResponse
from gamma_msg_srv.msg import Grasp
...
def srv_handle(req):
    ...
    index_mask = np.array(req.index_mask)
    # sample points
    ...
    index_mask = torch.from_numpy(index_mask[np.newaxis].astype(np.int32)).to(device)
    ...
    return GraspNetIndexListResponse(gg=gg_list)

if __name__=='__main__':
    net = get_net()
    rospy.init_node("GraspNetIndex_server")
    srv = rospy.Service("GraspNetIndex",GraspNetIndexList,srv_handle)
    print("Ready to get Grasp Index List.")
    rospy.spin()