...
from modules import ApproachNetIndex, CloudCrop, OperationNet, ToleranceNet
...
class GraspNetStage1Index(nn.Module):
    def __init__(self, input_feature_dim=0, num_view=300):
        super().__init__()
        self.backbone = Pointnet2Backbone(input_feature_dim)
        self.vpmodule = ApproachNetIndex(num_view, 256)
...
class GraspNetStage2Index(nn.Module):
    ...
    def forward(self, end_points):
        ...
        if self.is_training:
            ...
            grasp_top_views_rot = end_points['grasp_top_view_rot']
            seed_xyz = end_points['fp2_xyz_new']
...
class GraspNetIndex(nn.Module):
    def __init__(self, input_feature_dim=0, num_view=300, num_angle=12, num_depth=4, cylinder_radius=0.05, hmin=-0.02, hmax_list=[0.01,0.02,0.03,0.04], is_training=True):
        super().__init__()
        self.is_training = is_training
        self.view_estimator = GraspNetStage1Index(input_feature_dim, num_view)
        self.grasp_generator = GraspNetStage2Index(num_angle, num_depth, cylinder_radius, hmin, hmax_list, is_training)
...
def pred_decodeIndex(end_points):
    batch_size = len(end_points['point_clouds'])
    grasp_preds = []
    for i in range(batch_size):
        ## load predictions
        ...
        grasp_score = end_points['grasp_score_pred'][i].float()
        grasp_center = end_points['fp2_xyz_new'][i].float()
        approaching = -end_points['grasp_top_view_xyz'][i].float()
        grasp_angle_class_score = end_points['grasp_angle_cls_pred'][i]
        grasp_width = 1.2 * end_points['grasp_width_pred'][i]
        grasp_width = torch.clamp(grasp_width, min=0, max=GRASP_MAX_WIDTH)
        ...
        ## slice preds by objectness
        # objectness_score = end_points['objectness_score'][i].float() # (2, num_seed)
        # fp2_inds_new = end_points['fp2_inds_new'][i]
        # objectness_score = objectness_score[:,fp2_inds_new]
        # objectness_pred = torch.argmax(objectness_score, 0)
        # # end_points['fp2_inds'][i][torch.where(objectness_pred==1)[0]]
        # #
        # objectness_mask = (objectness_pred==1)
        # grasp_score = grasp_score[objectness_mask]
        # grasp_width = grasp_width[objectness_mask]
